...
public class WithdrawAccountRequestHandlerTest {
    Request request;
    EventBus eventBus;
    ...
    @Test
    public void shouldWithdrawAccountSuccessfully() throws Exception {
        UUID accountId = UUID.randomUUID();
        Mockito.when(request.getRequestBody()).thenReturn(new WithdrawAccountRequestBody(accountId, BigDecimal.ONE));
    ...
    public void shouldThrowExceptionIfAmountLessThanOREqualZero() throws Exception {
        UUID accountId = UUID.randomUUID();
        Mockito.when(request.getRequestBody()).thenReturn(new WithdrawAccountRequestBody(accountId, BigDecimal.ZERO));
        Mockito.when(accountProjection.getOne(accountId)).thenReturn(new Account(accountId, BigDecimal.ZERO));

        Assertions.assertThrows(Exception.class, () -> WithdrawAccountRequestHandler.handle(request, eventBus, accountProjection));
    }
    ...
    @Test
    public void shouldThrowExceptionIfAmountIsMoreThanAccountBalance() throws Exception {
        UUID accountId = UUID.randomUUID();
        Mockito.when(request.getRequestBody()).thenReturn(new WithdrawAccountRequestBody(accountId, BigDecimal.ONE));
        Mockito.when(accountProjection.getOne(accountId)).thenReturn(new Account(accountId, BigDecimal.ZERO));
    ...
    public void shouldUpdateRequestWithUpdatedAccount() throws Exception {
        UUID accountId = UUID.randomUUID();
        Mockito.when(request.getRequestBody()).thenReturn(new WithdrawAccountRequestBody(accountId, new BigDecimal(1)));
        Mockito.when(accountProjection.getOne(accountId)).thenReturn(new Account(accountId, new BigDecimal(100)));
        ...
        Assertions.assertEquals(new BigDecimal(99), account.getBalance());
    }
}
